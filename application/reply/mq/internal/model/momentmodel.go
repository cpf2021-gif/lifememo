...
type (
	...
	MomentModel interface {
		...
		AddMomentViewNum(ctx context.Context, id int64) error
		RandomMoments(ctx context.Context, limit int) ([]*Moment, error)
	}
...
func (m *customMomentModel) MomentsByUserId(ctx context.Context, userId, likeNum int64, status int, pubTime, sortField string, limit int) ([]*Moment, error) {
	...
	if sortField == "like_num" {
		anyField = likeNum
		query = fmt.Sprintf("select "+momentRows+" from "+m.table+" where author_id=? and status=? and like_num < ? order by %s desc, id desc limit ?", sortField)
	} else {
		anyField = pubTime
		query = fmt.Sprintf("select "+momentRows+" from "+m.table+" where author_id=? and status=? and publish_time < ? order by %s desc, id desc limit ?", sortField)
	}
...
func (m *customMomentModel) AddMomentViewNum(ctx context.Context, id int64) error {
	lifeMemoMomentIdKey := fmt.Sprintf("%s%v", cacheLifememoMomentMomentIdPrefix, id)
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (sql.Result, error) {
		query := fmt.Sprintf("update " + m.table + " set view_num=view_num+1 where id=?")
...
func (m *customMomentModel) RandomMoments(ctx context.Context, limit int) ([]*Moment, error) {
	var (
		err      error
		query    string
		articles []*Moment
	)
	query = fmt.Sprintf("select " + momentRows + " from " + m.table + " where status=2 order by rand() limit ?")
	err = m.QueryRowsNoCacheCtx(ctx, &articles, query, limit)
	if err != nil {
		return nil, err
	}
