...
var (
	...
	cacheLifememoReplyReplyIdPrefix = "cache:lifememoReply:reply:id:"
)
...
func (m *defaultReplyModel) Delete(ctx context.Context, id int64) error {
	lifememoReplyReplyIdKey := fmt.Sprintf("%s%v", cacheLifememoReplyReplyIdPrefix, id)
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, lifememoReplyReplyIdKey)
	return err
}
...
func (m *defaultReplyModel) FindOne(ctx context.Context, id int64) (*Reply, error) {
	lifememoReplyReplyIdKey := fmt.Sprintf("%s%v", cacheLifememoReplyReplyIdPrefix, id)
	var resp Reply
	err := m.QueryRowCtx(ctx, &resp, lifememoReplyReplyIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultReplyModel) Insert(ctx context.Context, data *Reply) (sql.Result, error) {
	lifememoReplyReplyIdKey := fmt.Sprintf("%s%v", cacheLifememoReplyReplyIdPrefix, data.Id)
...
func (m *defaultReplyModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheLifememoReplyReplyIdPrefix, primary)
}
