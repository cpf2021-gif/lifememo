...
var (
	...
	cacheLifememoLikeLikeCountIdPrefix         = "cache:lifememoLike:likeCount:id:"
	cacheLifememoLikeLikeCountBizIdObjIdPrefix = "cache:lifememoLike:likeCount:bizId:objId:"
)
...
func (m *defaultLikeCountModel) Delete(ctx context.Context, id int64) error {
	...
	lifememoLikeLikeCountIdKey := fmt.Sprintf("%s%v", cacheLifememoLikeLikeCountIdPrefix, id)
...
func (m *defaultLikeCountModel) FindOne(ctx context.Context, id int64) (*LikeCount, error) {
	lifememoLikeLikeCountIdKey := fmt.Sprintf("%s%v", cacheLifememoLikeLikeCountIdPrefix, id)
	var resp LikeCount
	err := m.QueryRowCtx(ctx, &resp, lifememoLikeLikeCountIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultLikeCountModel) FindOneByBizIdObjId(ctx context.Context, bizId string, objId int64) (*LikeCount, error) {
	lifememoLikeLikeCountBizIdObjIdKey := fmt.Sprintf("%s%v:%v", cacheLifememoLikeLikeCountBizIdObjIdPrefix, bizId, objId)
	var resp LikeCount
	err := m.QueryRowIndexCtx(ctx, &resp, lifememoLikeLikeCountBizIdObjIdKey, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v any) (i any, e error) {
...
func (m *defaultLikeCountModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheLifememoLikeLikeCountIdPrefix, primary)
}
